...
(T/F) Dynamic languages like Python don't check types at all, so no type errors can ever be caught before runtime.
...
In the Employee example, why was branching on the employee's **name** to decide the job title (using if/else) considered a code smell?
